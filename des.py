...
def main():
  ...
  if len(sys.argv) != 5:
    print 'Usage: python des.py encrypt <key> <plaintext> <ciphertext> \npython des.py decrypt <key> <ciphertext> <plaintext>'
    return

  keytext = str(sys.argv[2])
  print 'Using key ' + keytext
  for i in keytext:
    key.extend(to_binary[ord(i)])
  generate_keys()

  if sys.argv[1] == 'encrypt':
    plaintext_handle = open(sys.argv[3], 'r')
    ...
    ciphertext_handle = open(sys.argv[4], 'w')
    ciphertext_handle.write(hex_cipher)
    ciphertext_handle.close()
    print 'Encryption complete.'
      
  elif sys.argv[1] == 'decrypt':
    ciphertext_handle = open(sys.argv[3], 'r')
    cipher = ciphertext_handle.read()
    ciphertext_handle.close()
    ...
    ciphertext_handle = open(sys.argv[4], 'w')
    ciphertext_handle.write(text_mess)
    ciphertext_handle.close()
